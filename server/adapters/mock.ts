...
function buildReply(systemId: SystemId, request: AdapterRequest) {
  ...
  if (systemId === 'letta') {
    return `Tei님, 이 Conversation은 다른 아젠다와 분리해서 유지하겠습니다. 이 test mock은 native personal-memory 지속성·회상·삭제를 검증하지 않으므로 장기기억 상태는 UNKNOWN입니다. 방금 요청하신 “${latest.slice(0, 80)}”를 현재 Conversation의 중심 아젠다로 잡았습니다.`;
  }
