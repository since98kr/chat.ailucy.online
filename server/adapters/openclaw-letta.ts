  ...
  async *streamReply(request: AdapterRequest): AsyncGenerator<AdapterStreamItem> {
    ...
    while (true) {
      ...
      while (newline >= 0) {
        ...
        const delta = responseDelta(payload);
        if (delta) yield { type: 'delta', delta };
        // An OpenAI-compatible finish_reason terminates text generation, not the
        // SSE transport. Extension frames such as execution-evidence may follow;
        // keep reading until the actual [DONE] frame or stream EOF.
      }
