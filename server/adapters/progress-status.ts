...
const PRIVATE_WINDOWS_PATH = /\b[A-Za-z]:\\[^\s,;)}\]]+/g;
const RAW_ARGUMENT_BLOB = /(?:["']?\b(?:args|arguments|parameters|input)\b["']?\s*[:=]\s*[\[{]|\btool(?:_name)?\b\s*[:=].*\b(?:args|arguments|parameters|input)\b|\btool(?:_name)?\b\s*[:=]\s*\S+\s+(?:command|cmd|query|path|input|args|arguments|parameters)\s*[:=]|\btool\s+call\s+\S+\s*\(|(?:^|[\s=:])(?:\{|\[\s*\{)\s*(?:["'][^"']+["']|[A-Za-z_][\w.-]*)\s*:)/i;
