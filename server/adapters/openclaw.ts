...
function firstEnv(canonical: string, legacy: string) {
...
function openClawAgentTarget(value: string | undefined) {
  const target = boundedIdentifier(value ?? '', 'OPENCLAW_AGENT_TARGET');
  if (
    target !== 'openclaw'
    ...
    throw new Error('OPENCLAW_AGENT_TARGET must use an OpenClaw agent target such as openclaw/default or openclaw/<agentId>');
  }
  return target;
}
...
function responseFinished(payload: unknown) {
  ...
  return finishReason !== undefined && finishReason !== null && String(finishReason).trim() !== '';
}
...
async function serializeCurrentArtifacts(request: AdapterRequest, config: OpenClawConfig) {
...
export function openClawConfigFromEnv(): OpenClawConfig {
  const baseUrl = firstEnv('OPENCLAW_BASE_URL', 'LETTA_BASE_URL');
  if (!baseUrl) throw new Error('OPENCLAW_BASE_URL is required (legacy LETTA_BASE_URL is accepted only for migration compatibility)');
  return {
    baseUrl,
    chatPath: firstEnv('OPENCLAW_CHAT_PATH', 'LETTA_CHAT_PATH') || '/v1/chat/completions',
    healthPath: firstEnv('OPENCLAW_HEALTH_PATH', 'LETTA_HEALTH_PATH') || '/health',
    apiKey: firstEnv('OPENCLAW_API_KEY', 'LETTA_API_KEY'),
    agentTarget: openClawAgentTarget(firstEnv('OPENCLAW_AGENT_TARGET', 'LETTA_OPENCLAW_AGENT_TARGET')),
    sessionPrefix: firstEnv('OPENCLAW_SESSION_PREFIX', 'LETTA_OPENCLAW_SESSION_PREFIX') || 'chat-v2',
    timeoutMs: positiveInteger(firstEnv('OPENCLAW_TIMEOUT_MS', 'LETTA_TIMEOUT_MS'), 10_000, 'OPENCLAW_TIMEOUT_MS'),
    maxArtifactBytes: positiveInteger(
      firstEnv('OPENCLAW_MAX_ARTIFACT_BYTES', 'LETTA_MAX_ARTIFACT_BYTES'),
      10 * 1024 * 1024,
      'OPENCLAW_MAX_ARTIFACT_BYTES',
    ),
    maxArtifactTotalBytes: positiveInteger(
      firstEnv('OPENCLAW_MAX_ARTIFACT_TOTAL_BYTES', 'LETTA_MAX_ARTIFACT_TOTAL_BYTES'),
      20 * 1024 * 1024,
      'OPENCLAW_MAX_ARTIFACT_TOTAL_BYTES',
    ),
    artifactToolEnabled: enabled(firstEnv('OPENCLAW_ARTIFACT_TOOL_ENABLED', 'LETTA_ARTIFACT_TOOL_ENABLED')),
  };
}

export class OpenClawAdapter implements ChatBackendAdapter {
  readonly systemId = 'openclaw' as const;
  ...
  async *streamReply(request: AdapterRequest): AsyncGenerator<AdapterStreamItem> {
    const approved = approvedAdapterCapabilities(request, 'openclaw');
    ...
    if (approved.selectedAgent.agentId !== requestedSelectedAgentId) {
      throw new Error('OpenClaw transport selected-agent authorization mismatch');
    }
