...
EVIDENCE_DIR="${CHAT_STAGING_EVIDENCE_DIR:-test-results-staging-browser/openclaw}"

log() {
  printf '[chat-v2-openclaw] %s\n' "$*"
}
...
log 'Selecting the registered OpenClaw Lucy agent.'
curl -fsS -H "${AUTH_HEADER}" "${BASE}/api/agents" >"${TMP_DIR}/agents.json"
OPENCLAW_AGENT="$(python3 - "${TMP_DIR}/agents.json" <<'PY'
import json
import sys
...
agents = [a for a in payload.get('agents', [])
          if a.get('systemId') == 'openclaw'
          and a.get('enabled') is not False
          and a.get('directChatEnabled') is not False]
if not agents:
    raise SystemExit('no OpenClaw direct-chat agent')
...
[[ -n "${OPENCLAW_AGENT}" ]] || fail 'could not select OpenClaw Lucy'

log 'Creating one Chat Conversation for the two-turn OpenClaw session proof.'
CREATE_PAYLOAD="$(python3 - "${OPENCLAW_AGENT}" <<'PY'
import json, sys
print(json.dumps({'systemId':'openclaw','agentId':sys.argv[1],'title':'OpenClaw continuity staging proof'}))
PY
)"
...
log 'PASS: the same Chat Conversation preserved OpenClaw Lucy continuity.'
