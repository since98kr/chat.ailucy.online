...
log() {
  printf '[chat-v2-openclaw-benign-continuity] %s\n' "$*"
}
...
agents = [a for a in payload.get('agents', []) if a.get('systemId') == 'letta' and a.get('enabled') is not False and a.get('directChatEnabled') is not False]
if not agents:
    raise SystemExit('no Letta direct-chat agent')
agents.sort(key=lambda a: (0 if a.get('isLead') else 1, 0 if 'lucy' in ' '.join(str(a.get(k, '')) for k in ('id','displayName','shortName')).lower() else 1, int(a.get('sortOrder', 999999)), str(a.get('id', ''))))
print(agents[0]['id'])
PY
)"
...
import json, sys
print(json.dumps({'systemId':'letta','agentId':sys.argv[1],'title':'Benign OpenClaw continuity diagnostic'}))
PY
)"
...
log 'Turn 1: providing a harmless public test codename.'
...
log 'Turn 2: asking for the harmless public test codename from the previous turn.'
...
if marker not in text:
    raise SystemExit(f'benign OpenClaw session continuity marker missing; response={text!r}')
PY

log 'PASS: benign same-Conversation continuity is preserved through OpenClaw.'
