...
# Canonicalize the host's migration-era LETTA_* values before the candidate
# image is inspected. The legacy names are compatibility input only; the
# candidate runtime receives OPENCLAW_* exclusively.
...
ENV_VARS=(
  ...
  OPENCLAW_MAX_ARTIFACT_BYTES OPENCLAW_MAX_ARTIFACT_TOTAL_BYTES
  OPENCLAW_MAX_TEXT_ARTIFACT_BYTES OPENCLAW_NATIVE_BINARY_ARTIFACTS OPENCLAW_ARTIFACT_TOOL_ENABLED
  OPENCLAW_READINESS_PROBE_ENABLED OPENCLAW_READINESS_PROBE_MODEL OPENCLAW_READINESS_PROBE_PROMPT
  HERMES_BASE_URL HERMES_BASE_URL_MAP_JSON HERMES_CHAT_PATH HERMES_HEALTH_PATH HERMES_AGENT_ID HERMES_API_KEY HERMES_TIMEOUT_MS
