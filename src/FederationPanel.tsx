...
import { displayAgentId, displaySystemName } from './identity';
  ...
  return (
    <aside className="federation-panel" aria-label="교차 시스템 워크플로">
      <header className="federation-panel__header">
        <div><strong>Federated Workflow</strong><span>OpenClaw ↔ Hermes · explicit capsules</span></div>
      ...
      <section>
        ...
        <div className="federation-mode-row">
          ...
          <div>
            ...
            <small>{active ? `${displayAgentId(props.config?.coordinatorAgentId ?? '')} 조정 · ${props.config?.memoryPolicy}` : '시스템 간 문맥은 공유되지 않습니다.'}</small>
          </div>
        </div>
        ...
        {active && <div className="federation-lanes">{props.agents.filter((agent) => agent.enabled).map((agent) => <span key={agent.id} className={`lane-chip lane-chip--${agent.systemId}`}>{displayAgentId(agent.displayName)}<small>{displaySystemName(agent.systemId)}</small></span>)}</div>}
      </section>
      ...
      <section>
        ...
        <div className="capsule-list">{props.capsules.length === 0 ? <p className="federation-empty">Capsule이 없습니다.</p> : props.capsules.map((capsule) => <article className={`capsule-card capsule-card--${capsule.status}`} key={capsule.id}>
          <header><div><strong>{capsule.title}</strong><span>{displaySystemName(capsule.sourceSystemId)} → {displaySystemName(capsule.targetSystemId)}</span></div><em>{capsule.status}</em></header>
          <p>{capsule.content}</p>
      ...
      <section>
        ...
        <div className="workflow-run-list">{props.runs.map((run) => <button key={run.id} className={props.selectedRun?.id === run.id ? 'is-active' : ''} onClick={() => props.onSelectRun(run.id)}><span className={`run-dot run-dot--${run.status}`} /><span><strong>{run.status}</strong><small>{run.requestedAgentIds.map(displayAgentId).join(' · ')}</small></span></button>)}</div>
        {props.selectedRun && <div className="workflow-detail">
          ...
          {props.selectedRun.steps.map((step) => <div className={`workflow-step workflow-step--${step.status}`} key={step.id}><span className={`run-dot run-dot--${step.status}`} /><div><strong>{displayAgentId(step.agentId)}</strong><small>{displaySystemName(step.systemId)} · group {step.parallelGroup} · attempt {step.attempt}</small></div></div>)}
